...
func serveAgent() {
	...
	if err := generateWebConfig(serverAgent.GetPort()); err != nil {
		display.Errorf("⚠️  Failed to generate web config: %v\n", err)
	} else {
		display.Colorf(display.ColorGreen, "✅ Generated web/config.js with API URL\n")
	}

	spaHttpPort := env.GetEnvOrDefault("SPA_HTTP_PORT","8085")
	// Start the web UI server in a goroutine
	go func() {
		fs := http.FileServer(http.Dir("./web"))
		display.Colorf(display.ColorGreen, "✅ Web UI ready at http://localhost:%s\n", spaHttpPort)
		if err := http.ListenAndServe(":"+spaHttpPort, fs); err != nil {
...
// Application Title - configurable via environment variable SNIP_TITLE
window.SNIP_TITLE = '%s';
`, apiPort, env.GetEnvOrDefault("SNIP_TITLE", "🚀 Nova Crew Server Agent"))
